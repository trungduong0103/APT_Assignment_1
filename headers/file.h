...
bool validate_and_extract_data_from_column(const std::string &file_name, double array[], Column column) {
    int index = 0;
    std::ifstream infile(file_name);
    std::string line;
    getline(infile, line);
        ...
        switch (column) {
            case x:
                if (!string_is_digits(x_value)) {
                    std::cerr << "Parsing failed, data is in incorrect format, data: " << x_value << " at row " << index + 2
            ...
            case y:
                ...
                break;
            default:
                std::cerr << "Parsing failed, column must be of Column enum!" << std::endl;
                return false;
        }
        index++;
    }
